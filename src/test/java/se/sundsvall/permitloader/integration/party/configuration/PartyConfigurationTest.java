...
import static se.sundsvall.permitloader.integration.party.configuration.PartyConfiguration.REGISTRATION_ID;
...
class PartyConfigurationTest {
	...
	void testFeignBuilderCustomizer() {
		...
		try (final MockedStatic<FeignMultiCustomizer> feignMultiCustomizerMock = Mockito.mockStatic(FeignMultiCustomizer.class)) {
			...
			final var customizer = configuration.partyFeignBuilderCustomizer(clientRegistrationRepositoryMock);
